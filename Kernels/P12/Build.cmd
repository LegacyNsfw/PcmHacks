...
"%GCC_LOCATION%\m68k-elf-ld.exe" --section-start .kernel_code=0x%BASE_ADDRESS% -T kernel.ld %DUMP_MAP% -o kernel-p12.elf main.o write-kernel.o crc.o common.o common-readwrite.o flash-intel.o flash-amd.o
if %errorlevel% neq 0 goto :EOF

"%GCC_LOCATION%\m68k-elf-objcopy.exe" -O binary --only-section=.kernel_code --only-section=.rodata kernel-p12.elf kernel-p12.bin
if %errorlevel% neq 0 goto :EOF

if defined DUMP_ELF (
  "%GCC_LOCATION%\m68k-elf-objdump.exe" -d -S kernel-p12.elf > kernel-p12.disassembly
  if %errorlevel% neq 0 goto :EOF
)

if defined COPY_BIN (
  echo %BIN_LOCATION%
  copy kernel-p12.bin "%BIN_LOCATION%"
)
